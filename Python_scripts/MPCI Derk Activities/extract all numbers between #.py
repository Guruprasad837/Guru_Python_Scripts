...
# Function to extract all numbers between '#' and ':'
def extract_ids_between_hash_colon(text):
    if pd.isna(text):
        return None
    return re.findall(r'#(\d+):', str(text))  # Extract numbers between # and :

# Apply function to extract IDs
df['Extracted IDs'] = df[column_name].apply(extract_ids_between_hash_colon)
